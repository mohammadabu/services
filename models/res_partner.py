...
class ResPartner(models.Model):
    ...
    task_ids = fields.One2many('services.task', 'partner_id', string='Tasks')
    task_count = fields.Integer(compute='_compute_task_count', string='# Tasks')

    def _compute_task_count(self):
        fetch_data = self.env['services.task'].read_group([('partner_id', 'in', self.ids)], ['partner_id'], ['partner_id'])
